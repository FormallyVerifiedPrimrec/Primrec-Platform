...
- **Database (Supabase):**
  - Ensure the `profiles` table has columns for `name` and `profile_picture`.
  - The `profile_picture` can just be the picture encoded as a string
- **Custom API:**
  - `GET /api/users/me` - Fetch the current user's profile data.
  - `PATCH /api/users/me` - Endpoint to update `name` and `profile_picture`.

### 2. Immutable Submissions

- **Database (Supabase):**
  - _Note: We can use the existing `best_submission_source` column in the `user_interactions` table._
...
- **Custom API:**
  ...
  - New Endpoint `GET /api/challenges/:id/solutions`:
    ...
    - **Security:** This API _must_ check if the requesting user has `has_solved = true` for this challenge (or is the host) before returning the data. If not, return 403 Forbidden.
